...
//				}
                ...
                {
                    resortTiles = false;
                    loadQueue.Sort((TileInfo x, TileInfo y) => { return x.distToCamera.CompareTo(y.distToCamera); });
                }
            ...
            if (!cornersOccluded)
            {
                ...
                for (int c = 0; c < 4; c++)
                {
                    ...
                    insideViewport = insideViewport ||
                                     (scrPos.z > 0 && scrPos.x >= 0 && scrPos.x < 1f && scrPos.y >= 0 && scrPos.y < 1);
                    if (scrPos.x < minX)
                        minX = scrPos.x;
                    if (scrPos.x > maxX)
                        maxX = scrPos.x;
...
//				}
                ...
                float aparentSize = Mathf.Max((maxX - minX) * (float) Camera.main.pixelWidth,
                    (maxY - minY) * (float) Camera.main.pixelHeight);
...
//					}
                    ...
                    ti.renderer.enabled = !allLoaded &&
                                          (ti.loadStatus == TILE_LOAD_STATUS.Loaded || zoomLevel < TILE_MIN_ZOOM_LEVEL);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            ti.distToCamera = (ti.cornerWorldPos[0] - Camera.main.transform.position).sqrMagnitude *
                                              ti.zoomLevel;
...
//			}
        ...
        void GetTileFromLatLon(int zoomLevel, float lat, float lon, out int xtile, out int ytile)
        {
            ...
            ytile = (int) ((1.0 - Math.Log(Math.Tan(lat * Math.PI / 180.0) + 1.0 / Math.Cos(lat * Math.PI / 180.0)) /
                            Math.PI) / 2.0 * (1 << zoomLevel));
        }

        public Vector2 GetLatLonFromTile(float x, float y, int zoomLevel)
        {
        ...
        internal IEnumerator LoadTileContentBackground(TileInfo ti)
        {
            ...
            if (ti.texture.width <= 16 && ti.loadedFromCache)
            {
        ...
        void CreatePole(TileInfo ti)
        {
            ...
            Renderer obj = CreateObject(tilesRoot.transform, name, vertices.ToArray(), indices.ToArray(), uv.ToArray(),
                ti.parent.normalMat, 0);
            if (isNorth)
            {
                northPoleObj = obj;
            }
